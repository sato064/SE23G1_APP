...
df = pd.DataFrame({'第1回機能仕様書インスペクション結果': srs1,'第2回機能仕様書インスペクション結果': srs2,'第1回クラス図インスペクション結果': c1,'第2回クラス図インスペクション結果': c2,'第1回画面シナリオインスペクション結果': ui1,'第1回DB設計書インスペクション結果': db1, '第1回シーケンス図インスペクション結果': seq},
                    index=["不足・不明", "誤り", "改善要求", "文書形式・誤字脱字・命名規則"])

st.title("第1回シーケンス図インスペクション`の結果") # タイトル
st.subheader('指摘数')
st.table(df.T) # 表の表示
st.subheader('指摘数の割合')
